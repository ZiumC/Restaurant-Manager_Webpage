...
namespace Restaurants_Webpage.Controllers
{
    public class OwnerController : Controller
    {
        ...
        private readonly string _assignEmployeeToRestaurantUrl;
        ...
        public OwnerController(IConfiguration config)
        {
            ...
            string assignEmployeeToRestaurantUrl = restaurantBaseUrl + "/{0}" + _config["Endpoints:Paths:Employee"] + "/{1}" + _config["Endpoints:Paths:Type"] + "/{2}";
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(assignEmployeeToRestaurantUrl))
                {
                    throw new Exception("Url for assigning employee to restaurant can't be empty");
                }
            ...
            _assignEmployeeToRestaurantUrl = assignEmployeeToRestaurantUrl;
        ...
        public async Task<IActionResult> RemoveEmployeeFromRestaurant(int idEmployee, int idRestaurant)
        {
            ...
            return RedirectToAction("employment", "owner", new { idEmployee });
        }
        ...
        public async Task<IActionResult> AssignEmployeeToRestaurant(int idEmployee, int idRestaurant, int idType)
        {
            ...
            if (idEmployee <= 0 || idRestaurant <= 0 || idType <= 0)
            {
                TempData["ActionFailed"] = "Did you modified request?";
                return RedirectToAction("restaurants", "restaurant");
            }
            ...
            string url = string.Format(_assignEmployeeToRestaurantUrl, idRestaurant, idEmployee, idType);
            var response =
                await HttpRequestUtility.SendSecureRequestJwtAsync(url, Utils.HttpMethods.POST, null, jwtUtils.GetJwtRequestCookie());
            if (response == null)
            {
                TempData["ActionFailed"] = "Unable connect to server the external server, please try again later.";
                return RedirectToAction("restaurants", "restaurant");
            ...
            if (response.IsSuccessStatusCode)
            {
                TempData["ActionSucceeded"] = $"Employee has been successfully assigned to the restaurant!";
            }
            else
            {
                TempData["ActionFailed"] = await HttpRequestUtility.GetResponseMessage(response);
